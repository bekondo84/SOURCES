 ...
 * @since Mon Jan 07 12:29:53 WAT 2019
 * 
 */
@Path("/userautorisation")
public class UserAutorisationRSImpl
    extends AbstractGenericService<UserAutorisation, Long>
    implements UserAutorisationRS
{
     ...
     * 
     */
    @Manager(application = "kerencore", name = "UserAutorisationManagerImpl", interf = UserAutorisationManagerRemote.class)
    protected UserAutorisationManagerRemote manager;
    
    @Manager(application = "kerencore", name = "GroupeManagerImpl", interf = GroupeManagerRemote.class)
    protected GroupeManagerRemote grpmanager;

    public UserAutorisationRSImpl() {
        super();
    }
     ...
     */
    @Override
    public GenericManager<UserAutorisation, Long> getManager() {
        return manager;
    }
    ...
    public RSNumber count(HttpHeaders headers) {
        ...
        RestrictionsContainer container = RestrictionsContainer.newInstance();  
         ...
         //Filter
        RSNumber number = new RSNumber(grpmanager.count(container.getPredicats()));
//        System.out.println(AbstractGenericService.class.toString()+".count === "+" == "+number.getValue());
        return number;
    }

    @Override
    public List<UserAutorisation> filter(HttpHeaders headers, int firstResult, int maxResult) {
         Gson gson =new Gson();
        //Type predType = ;
        List contraints = new ArrayList();
...
//        System.out.println(AbstractGenericService.class.toString()+" === "+headers.getRequestHeader("predicats")+" === "+firstResult+" === "+maxResult+" == "+contraints);   
        ...
        List<Groupe> groupes = grpmanager.filter(container.getPredicats(), null , new HashSet<String>(), firstResult, maxResult);
        List<UserAutorisation> datas = new ArrayList<UserAutorisation>();
        for(Groupe grp:groupes){
            datas.add(new UserAutorisation(grp, null));
        }//end for(Groupe grp:groupes){
        return datas;
    }
