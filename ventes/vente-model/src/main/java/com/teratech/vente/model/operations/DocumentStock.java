...
public class DocumentStock extends BaseElement implements Serializable,Comparable<DocumentStock>{
    ...
    @Predicate(label = "Client",type = Tier.class,target = "many-to-one",search = true)
    @Filter("[{\"fieldName\":\"type\",\"value\":\"0\"}]")
    ...
    @Predicate(label = "Adresse livraison",search = true,optional = false)
...
//    protected List<LigneDocumentStock> lignes = new ArrayList<LigneDocumentStock>();
    ...
    public DocumentStock(DocumentStock doc) {
        ...
        this.lieu = doc.lieu;
        if(doc.client!=null){
            this.client = new Tier(doc.client);
        }
    }
