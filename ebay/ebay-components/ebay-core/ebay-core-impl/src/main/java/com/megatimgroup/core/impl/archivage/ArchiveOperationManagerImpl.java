
package com.megatimgroup.core.impl.archivage;

import com.bekosoftware.genericdaolayer.dao.ifaces.GenericDAO;
import com.bekosoftware.genericmanagerlayer.core.impl.AbstractGenericManager;
import com.megatim.common.annotations.Inject;
import com.megatim.common.annotations.Transaction;
import com.megatimgroup.core.ifaces.archivage.ArchiveOperationManager;
import com.megatimgroup.model.archivage.ArchiveOperation;


/**
 * Classe d'implementation du manager
 * @since Tue May 02 14:26:30 WAT 2017
 * 
 */
@Transaction
public class ArchiveOperationManagerImpl
    extends AbstractGenericManager<ArchiveOperation, String>
    implements ArchiveOperationManager 
{

    /**
     * On injecte la DAO
     * 
     */
    @Inject("com.megatimgroup.dao.impl.archivage.ArchiveOperationDAOImpl")
    protected GenericDAO dao;

    public ArchiveOperationManagerImpl() {
    }

    /**
     * Methode permettant de retourner la DAO
     * 
     */
    @Override
    public GenericDAO<ArchiveOperation, String> getDao() {
        return dao;
    }

    /**
     * Methode permettant de retourner l'id de l'entite
     * 
     */
    @Override
    public String getEntityIdName() {
        return "code";
    }

}
