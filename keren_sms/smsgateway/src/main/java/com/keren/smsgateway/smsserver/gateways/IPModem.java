...
package com.keren.smsgateway.smsserver.gateways;

import com.keren.smsgateway.model.AGateway.Protocols;
import com.keren.smsgateway.modem.IPModemGateway;
import com.keren.smsgateway.modem.ModemGateway;
import com.keren.smsgateway.modem.ModemGateway.IPProtocols;
import com.keren.smsgateway.smsserver.SMSServer;
...
{
	public IPModem(String myGatewayId, Properties myProps, SMSServer myServer)
	{
	...
	public void create() throws Exception
	{
		...
		setGateway(new IPModemGateway(getGatewayId(), getProperties().getProperty(propName + "ip"), Integer.parseInt(getProperties().getProperty(propName + "port")), getProperties().getProperty(propName + "manufacturer"), getProperties().getProperty(propName + "model")));
		...
		((IPModemGateway) getGateway()).setSimPin(getProperties().getProperty(propName + "pin"));
