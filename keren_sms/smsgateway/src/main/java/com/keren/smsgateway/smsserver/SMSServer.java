...
package com.keren.smsgateway.smsserver;

import com.keren.smsgateway.balancing.LoadBalancer;
import com.keren.smsgateway.model.ICallNotification;
import com.keren.smsgateway.model.IInboundMessageNotification;
import com.keren.smsgateway.model.IOutboundMessageNotification;
import com.keren.smsgateway.model.IQueueSendingNotification;
import com.keren.smsgateway.model.InboundMessage;
import com.keren.smsgateway.model.InboundMessage.MessageClasses;
import com.keren.smsgateway.model.Library;
...
import com.keren.smsgateway.routing.Router;
import com.keren.smsgateway.smsserver.gateways.AGateway;
import com.keren.smsgateway.smsserver.interfaces.Interface;
import com.keren.smsgateway.smsserver.interfaces.Interface.InterfaceTypes;
...
public class SMSServer
{
	...
	private void loadConfiguration() throws Exception
	{
		...
		if (getProperties().getProperty("smsserver.balancer", "").length() > 0)
		{
			try
			{
				...
				Class<?> c = Class.forName((getProperties().getProperty("smsserver.balancer", "").indexOf('.') == -1 ? "com.keren.smsgateway.balancing." : "") + getProperties().getProperty("smsserver.balancer", ""));
				Constructor<?> constructor = c.getConstructor(argsClass);
				LoadBalancer balancer = (LoadBalancer) constructor.newInstance(args);
		...
		if (getProperties().getProperty("smsserver.router", "").length() > 0)
		{
			try
			{
				Object[] args = new Object[] { getService() };
				Class<?>[] argsClass = new Class[] { Service.class };
				Class<?> c = Class.forName((getProperties().getProperty("smsserver.router", "").indexOf('.') == -1 ? "com.keren.smsgateway.routing." : "") + getProperties().getProperty("smsserver.router", ""));
				Constructor<?> constructor = c.getConstructor(argsClass);
				Router router = (Router) constructor.newInstance(args);
		...
		for (int i = 0; i < Integer.MAX_VALUE; i++)
		{
			try
			{
				...
				Class<?> c = Class.forName((gtwClass.indexOf('.') == -1 ? "com.keren.smsgateway.smsserver.gateways." : "") + gtwClass);
				Constructor<?> constructor = c.getConstructor(argsClass);
		...
		for (int i = 0; i < Integer.MAX_VALUE; i++)
		{
			try
			{
				...
				Class<?> c = Class.forName((infClass.indexOf('.') == -1 ? "com.keren.smsgateway.smsserver.interfaces." : "") + infClass);
				Constructor<?> constructor = c.getConstructor(argsClass);
	...
	void sendMessages() throws Exception
	{
		...
		for (com.keren.smsgateway.model.AGateway gtw : getService().getGateways())
			if (gtw.isOutbound())
			{
