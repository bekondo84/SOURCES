...
import com.keren.smsgateway.modem.SerialModemGateway;
...
{
	public SerialModem(String myGatewayId, Properties myProps, SMSServer myServer)
	{
	...
	public void create() throws Exception
	{
		...
		setGateway(new SerialModemGateway(getGatewayId(), getProperties().getProperty(propName + "port"), Integer.parseInt(getProperties().getProperty(propName + "baudrate")), getProperties().getProperty(propName + "manufacturer"), getProperties().getProperty(propName + "model")));
		...
		((SerialModemGateway) getGateway()).setSimPin(getProperties().getProperty(propName + "pin"));
