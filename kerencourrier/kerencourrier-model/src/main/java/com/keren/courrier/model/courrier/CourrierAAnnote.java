...
public class CourrierAAnnote extends BaseElement implements Serializable, Comparable<CourrierAAnnote> {
	...
	@Predicate(label = "Mention du courrier", target = "combobox", values = "Ordinaire;Confidentiel", editable = false, search = true)
	...
	@Column(name = "T_CAT")
	@Predicate(label = "Catégorie Courrier", optional = false, search = false, target = "combobox", values = "Courrier Arrivée;Courrier Départ;Courrier Interne;Document GED", editable = false)
	...
	@Column(name = "D_ARR")
	@Predicate(label = "Date d'arrivée Courrier", optional = true, updatable = false, search = false, editable = false, type = Date.class, target = "date")
	@Temporal(javax.persistence.TemporalType.DATE)
	private Date darrive;

	@ManyToOne
	@JoinColumn(name = "T_CORRES")
	@Predicate(label = "Expéditeur", type = Correspondant.class, target = "many-to-one", search = true, editable = false, optional = false)
	...
	@Predicate(label = "Pièces jointes", type = FichierLie.class, target = "one-to-many", edittable = true, editable = false, group = true, groupName = "group1", groupLabel = "objet/Pièces jointes")
	...
	private CourrierAAnnote origanal;
	...
	public CourrierAAnnote(String categorie, String niveauDeTraitement, TypeCourrier typecourrier, Priorite priorite,
	 ...
	 * 
	 */
	public CourrierAAnnote() {
		// TODO Auto-generated constructor stub
	}

	public CourrierAAnnote(CourrierAAnnote dep) {
		...
		// this.origanal = new CourrierAAnnote(dep.origanal);
		// }
		// if(dep.bordero!=null){
	...
	public CourrierAAnnote(CourrierDepart dep) {
	...
	public CourrierAAnnote(Courrier dep) {
		super(dep.getId(), dep.getDesignation(), dep.getModuleName(), dep.getCompareid());
		this.categorie = dep.getCategorie();
		this.code = dep.getCode();
	...
	public CourrierAAnnote(CourrierInterne dep) {
		super(dep.getId(), dep.getDesignation(), dep.getModuleName(), dep.getCompareid());
		this.categorie = dep.getCategorie();
		this.code = dep.getCode();
	...
	public CourrierAAnnote getOriganal() {
		return origanal;
	}

	public void setOriganal(CourrierAAnnote origanal) {
	...
	@Override
	public int compareTo(CourrierAAnnote o) {
		// TODO Auto-generated method stub
		return code.compareTo(o.code);
	}

	public CourrierAAnnote newInstace() {
		CourrierAAnnote entity = new CourrierAAnnote();
		...
		if (this.origanal != null) {
			entity.origanal = new CourrierAAnnote(this.origanal);
		}
